 ...
 * scope), with everything the UI needs to display + edit it. */
export interface PathInfo {
  /** Full key as it appears in `get_readings` / `vt.dump`, e.g.
   * "vt-demo.controller.pid.kp" when going through an scope. */
  fullPath: string;
   ...
   * scope; the resource name itself when direct). */
  source: string;
...
export interface ConnectionConfig {
  ...
  keyId: string;
  apiKey: string;
  /** Resource name to query. Typically the scope. */
  resource: string;
  /** "auto" probes vt.schema_all first (scope), falls back to vt.schema
   * (direct sensor). User can force either with "scope" or "direct". */
